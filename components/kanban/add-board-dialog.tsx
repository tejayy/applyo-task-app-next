...
interface AddBoardDialogProps {
  ...
  onAddBoard: (title: string, color: string) => void
}
...
export function AddBoardDialog({ open, onOpenChange, onAddBoard }: AddBoardDialogProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (title.trim()) {
      onAddBoard(title.trim(), selectedColor)
      setTitle("")
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Create a new board to organize your tasks. Give it a meaningful title and pick a color that represents its purpose.
          </DialogDescription>
        </DialogHeader>
